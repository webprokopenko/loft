...
io.sockets.on('connection', function(socket) {
  ...
  // функция вовзращает массив имен, кто находится в комнате room
  ...
  // событие наступатет когда пользователь впервые заходит в чат
  ...
  socket.on('switchroom', function(newroom) {
    ...
    // в новой комнате формируем новый список пользователей комнаты другим пользователям
    socket.broadcast
      .to(newroom)
      .emit('updateusers', updateUsers(newroom));
    // в старой комнате формируем новый список пользователей комнаты другим пользователям
    socket.broadcast
      .to(oldRoom)
  ...
  socket.on('message', function(data) {
    // возвращаем пользователю, что он написал обратно
    socket
      .json.emit('updatechat', { name: users[socket.id].name, msg: data });
